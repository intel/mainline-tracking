...
void asm_fred_entry_from_kvm(struct fred_ss ss, unsigned int vector);
...
static __always_inline void fred_nmi_from_kvm(unsigned int vector)
{
	struct fred_ss ss = {
		.ss	= __KERNEL_DS,
		.type	= EVENT_TYPE_NMI,
	...
	asm_fred_entry_from_kvm(ss, vector);
}
