...
#define word_for_each_set_bit(bit, val)			 \
	unsigned long long __v = (val);			 \
	for (; __v && ((bit = __builtin_ctzll(__v)), 1); \
	     __v &= __v - 1)
...
{
	return vec_qwords || vec_mask || pred_qwords || pred_mask ?
		-EINVAL : 0;
}
