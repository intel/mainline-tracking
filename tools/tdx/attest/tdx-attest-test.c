...
{
	__u8 *quote_data, report_data[TDX_REPORT_DATA_LEN];
	__u64 quote_size;
	int ret;

	ret = get_tdreport(devfd, dump_data, report_data);
	if (ret) {
		printf("TDX_CMD_GET_TDREPORT ioctl() %d failed\n", ret);
		return ret;
	}
	...
	quote_data = mmap(NULL, quote_size, PROT_READ, MAP_SHARED, devfd, 0);
	if (quote_data == MAP_FAILED) {
		printf("%s mmap failed\n", devname);
		return -ENOMEM;
	}

	ret = ioctl(devfd, TDX_CMD_GEN_QUOTE, report_data);
	if (ret) {
