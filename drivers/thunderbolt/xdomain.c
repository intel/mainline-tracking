 ...
 * guaranteed to be be within range supported by the input lane adapter.
...
int tb_xdomain_alloc_in_hopid(struct tb_xdomain *xd, int hopid)
{
	...
	return ida_simple_get(&xd->in_hopids, hopid, xd->local_max_hopid,
			      GFP_KERNEL);
}
 ...
 * Returns allocated HopID or negative errno. Specifically returns
 * %-ENOSPC if there are no more available HopIDs. Returned HopID is
 * guaranteed to be be within range supported by the output lane adapter.
 * Call tb_xdomain_release_in_hopid() to release the allocated HopID.
 */
int tb_xdomain_alloc_out_hopid(struct tb_xdomain *xd, int hopid)
{
	...
	return ida_simple_get(&xd->out_hopids, hopid, xd->remote_max_hopid,
			      GFP_KERNEL);
}
...
{
	ida_simple_remove(&xd->in_hopids, hopid);
}
...
{
	ida_simple_remove(&xd->out_hopids, hopid);
}
 ...
 * @receive_ring: DMA ring used to to receive packets from @receive_path
 *
