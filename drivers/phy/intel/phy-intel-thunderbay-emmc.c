...
#define is_caldone(phy) check_status(phy, CAL_DONE)
#define is_dllrdy(phy)  check_status(phy, DLL_RDY)
 ...
 * 1 --> actual settings
 */
static int phy_power_sts = PHY_UNINITIALIZED;
...
static int check_status(struct thunderbay_emmc_phy *tbh_phy, u32 flag)
{
	int retries = 2000;
	u32 val;
...
static int thunderbay_emmc_phy_power_on(struct phy *phy)
{
	...
	if (phy_power_sts == PHY_UNINITIALIZED) {
	...
	} else if (phy_power_sts == PHY_INITIALIZED) {
	...
	/* Reset, init seq will be called without phy_power_off, so this indicates init seq */
		phy_power_sts = PHY_UNINITIALIZED;
	}
