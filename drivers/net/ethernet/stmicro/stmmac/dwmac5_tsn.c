...
const struct tsn_error_desc dwmac5_tsn_err_desc[STMMAC_TSN_STAT_SIZE] = {
	{ true, "BTRE" },  /* BTR Error */
			  ...
			  u32 txqcnt)
{
	...
	if (status & MTL_EST_STATUS_BTRE) {
		if ((status & MTL_EST_STATUS_BTRL) ==
			...
			err_stat->count[EST_ERR_BTRLM]++;
		else
			err_stat->count[EST_ERR_BTRE]++;
...
void dwmac510_tsnif_setup(struct mac_device_info *mac)
{
	...
	mac->tsn_info.err_desc = &dwmac5_tsn_err_desc[0];
}
