...
void tsn_est_irq_status(struct mac_device_info *hw, struct net_device *dev)
{
	...
	status = tsnif_est_irq_status(hw, ioaddr, dev, &info->err_stat,
				      info->cap.txqcnt);
}

int tsn_err_dump(struct mac_device_info *hw,
		 int index, unsigned long *count, const char **desc)
{
	struct tsnif_info *info = &hw->tsn_info;
	const struct tsn_error_desc *err_desc;
	unsigned long *ptr;

	ptr = (unsigned long *)&info->err_stat;
	err_desc = info->err_desc;

	if (!(err_desc + index)->valid)
		return -EINVAL;
	if (count)
		*count = *(ptr + index);
	if (desc)
		*desc = (err_desc + index)->desc;
	return 0;
}
